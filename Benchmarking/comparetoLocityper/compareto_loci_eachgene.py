...
file2 = "/Users/wangfeima/Documents/MarkLab/Ctyper/Locityper/locityper-benchmarking/QV/full_illumina.csv"


file1 = "/Users/wangfeima/Documents/MarkLab/Ctyper/Locityper/nonleaves_lociresults_273CMR_withname.txt"
...
for genes,benchs in ctypergenes.items():
	...
	if name in useprefix and len([x for x in genes if x in allgenes]):
		...
		print(";".join(genes),benchs_zeros, loci_zeros,'{:.2e}'.format(benchs_mean), '{:.2e}'.format(loci_mean))
		
	elif name in useprefix:
		
				
		allgenesset.update(genes)
		benchs_zeros = benchs.count(0.0)/len(benchs)
		
		benchs_mean = sum(benchs)/len(benchs)
		
		print(";".join(genes),benchs_zeros, "NA",'{:.2e}'.format(benchs_mean), "NA")
