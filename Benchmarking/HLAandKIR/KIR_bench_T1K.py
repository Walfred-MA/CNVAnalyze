...
with open(immanno_file, mode = 'r') as f:
	...
	for line in f:
		...
		allpredix.add(prefix)
		genes = line[1].split(",") 
		
		imm_data[line[0]].extend([x for x in genes if x.startswith(prefix)])
		lr_counts[line[0].split("_")[2]].extend([x for x in genes if x.startswith(prefix)])
...
signifi = 0
allhwe = []
fullfolder = '/Users/walfred/Documents/Marklab/HLA/T1Ks_KIR/'

allfiles = [fullfolder+x for x in os.listdir(fullfolder) if x.endswith("_allele.tsv")]
