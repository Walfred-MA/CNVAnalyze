...
def HLAcomp(list1, list2):
	...
	common, uncommon = find_common(list1, list2)
	common_2, uncommon_2 = find_common(list2, list1)
	
	return min(len(common), len(common_2)), len(uncommon), max(0,len(common)-len(common_2)),max(0,len(common_2)-len(common))

def HLAcomp2(list1, list2):
	
	
	list1 = [":".join(x.split(":")[0:2]) for x in list1]
	list2 = [":".join(x.split(":")[0:2]) for x in list2]
...
signifi = 0
allhwe = []
fullfolder = '/Users/walfred/Documents/Marklab/HLA/fullset/'

allfiles = [fullfolder+x for x in os.listdir(fullfolder)if x.split(".")[0] in hprc]

results = cl.defaultdict(lambda: [0,0,0,0,0,0,0,0,0])

gp_counts = []

tp0_all, fn0_all, tp1_all, fn1_all, tp2_all, fn2_all = 0, 0 ,0 , 0 , 0 ,0
cnp0_all, cnp1_all, cnp2_all = 0 , 0 , 0
cnn0_all, cnn1_all, cnn2_all = 0 , 0 , 0
for file in allfiles:
	...
	with open(file, mode = 'r') as f:
		...
		for line in f:
			...
			if len(line) < 2 or line[1].startswith("HLA") == False:
				continue
	...
	for atype in alltypes:
		...
		tp0, fn0, cnn0,cnp0 = HLAcomp(gp_counts_type, lr_counts_type)
		results[atype][0] += tp0
		results[atype][1] += cnn0
		results[atype][2] += cnp0
		
		tp1, fn1, cnn1,cnp1 = HLAcomp2(gp_counts_type, lr_counts_type)
		results[atype][3] += tp1
		results[atype][4] += cnn1
		results[atype][5] += cnp1
		
		tp2, fn2, cnn2,cnp2= HLAcomp3(gp_counts_type, lr_counts_type)
		results[atype][6] += tp2
		results[atype][7] += cnn2
		results[atype][8] += cnp2
...
print(tp2_all, cnn2_all, cnp2_all)
cnp0_all, cnp1_all, cnp2_all = 0 , 0 , 0
cnn0_all, cnn1_all, cnn2_all = 0 , 0 , 0
for atype in sorted(list(results.keys())):
	...
	tp0, cnn0, cnp0, tp1, cnn1, cnp1, tp2, cnn2, cnp2 = results[atype]
	
	print(atype ,cnn2/(tp2+cnn2), cnp2/(tp2+cnn2) )



print("LOO \n\n")
LOOfolder = '/Users/walfred/Documents/Marklab/HLA/LOO/'
...
results = cl.defaultdict(lambda: [0,0,0,0,0,0,0,0,0])
tp0_all, fn0_all, tp1_all, fn1_all, tp2_all, fn2_all,cnn2_all, cnp2_all = 0, 0 ,0 , 0 , 0 ,0,0,0
