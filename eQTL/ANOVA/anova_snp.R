randomAssignSNP = function(df)
{
        ...
        for (i in 1:nrow(table)) {
                ...
                freq <- sum(table[i, ])/(2*colnum)  # Get row sum from the original table
                random_row <- sample(c(0, 1, 2), size = colnum, replace = TRUE, prob = c((1-freq)^2, 2* freq*(1-freq), freq ^ 2))
                # Assign the random integers to the corresponding row in the random table
                random_table[i,] = random_row
        }
...
model_randomtype_residuel = trails(model_formula, df, randomSample , 100)
explained_ss_randomtype =  total_ss- model_randomtype_residuel




cat( input_file,nrow(df),ncol(df)-1  ,total_ss, explained_variance , explained_ss_randomtype, 0.0,sep=',') 
